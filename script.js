...
document.getElementById('hse-form').addEventListener('submit', async (e) => {
    ...
    const reportData = {
        ...
        fotoAdmin: await getBase64(document.getElementById('fotoAdmin').files[0]) || (index > -1 ? reports[index].fotoAdmin : null),
        fotoInsiden: await getBase64(document.getElementById('fotoInsiden').files[0]) || (index > -1 ? reports[index].fotoInsiden : null)
    };

    if (index > -1) reports[index] = reportData;
    else reports.push(reportData);
