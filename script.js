...
function hideForm() {
    if(confirm("Batalkan pengisian laporan?")) {
...
// 9. DOWNLOAD FILTERED PDF (REKAP HARIAN/MINGGUAN/BULANAN)
async function downloadFilteredPDF(range) {
    ...
    // Sort descending agar laporan terbaru di halaman awal rekap
    ...
    for (let i = 0; i < filteredData.length; i++) {
        ...
        // Header
        doc.setFontSize(16); doc.setFont("helvetica", "bold");
        doc.text(`DAILY HSE REPORT (${range.toUpperCase()})`, 105, 15, { align: "center" });
        doc.setFontSize(10); doc.setFont("helvetica", "normal");
        ...
        const addImgPDF = (imgData, x, y, maxW, maxH, label) => {
            if (!imgData) return 0;
            try {
                const props = doc.getImageProperties(imgData);
                const ratio = props.width / props.height;
        ...
        let h1 = addImgPDF(d.fotoAdmin, 20, currentY, 80, 55, "Foto Aktivitas Utama:");
        if (isIncident && d.fotoInsiden) {
            addImgPDF(d.fotoInsiden, 110, currentY, 80, 55, "Foto Temuan Insiden:");
        }
    }
