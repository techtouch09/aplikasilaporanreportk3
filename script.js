...
function generateHSEReportPage(doc, data, titleText) {
    ...
    const addAdaptiveImg = (imgData, label, yPos) => {
        ...
        try {
            ...
            let finalHeight = fullWidth / ratio;
            const bottomMargin = 280;
            const availableSpace = bottomMargin - yPos;

            if (finalHeight > availableSpace && availableSpace > 60) {
                finalHeight = availableSpace - 10;
                doc.setFont("times", "bold");
                doc.text(label, 15, yPos);
                doc.addImage(imgData, 'JPEG', 15, yPos + 2, finalHeight * ratio, finalHeight);
                return yPos + finalHeight + 15;
            } else if (availableSpace <= 60) {
                doc.addPage();
                yPos = 20;
                doc.setFont("times", "bold");
                doc.text(label, 15, yPos);
                doc.addImage(imgData, 'JPEG', 15, yPos + 2, fullWidth, fullWidth / ratio);
                return yPos + (fullWidth / ratio) + 15;
            } else {
                doc.setFont("times", "bold");
                doc.text(label, 15, yPos);
                doc.addImage(imgData, 'JPEG', 15, yPos + 2, fullWidth, finalHeight);
                return yPos + finalHeight + 15;
            }
        } catch (e) { return yPos; }
    };
...
// 5. DOWNLOAD FUNCTIONS (DENGAN PERBAIKAN NAMA FILE)
async function downloadPDF(index) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF('p', 'mm', 'a4');
    const data = reports[index];
    generateHSEReportPage(doc, data, "DAILY HSE REPORT");
    
    // Format tanggal untuk nama file (DD-MM-YYYY)
    const fileDate = formatDateIndo(data.tgl).replace(/\//g, '-');
    doc.save(`HSE_REPORT_${data.proyek.toUpperCase()}_${fileDate}.pdf`);
}
...
async function downloadFilteredPDF(range) {
    ...
    const filteredData = reports.filter(item => {
        ...
        if (range === 'bulanan') return reportDate.getMonth() === now.getMonth() && reportDate.getFullYear() === now.getFullYear();
        return false;
    });

    if (filteredData.length === 0) {
        alert(`Tidak ada laporan untuk periode ${range}.`);
        return;
    }

    filteredData.sort((a, b) => new Date(`${b.tgl}T${b.waktu}`) - new Date(`${a.tgl}T${a.waktu}`));
    
    filteredData.forEach((data, i) => {
        if (i > 0) doc.addPage();
        generateHSEReportPage(doc, data, `REKAP HSE (${range.toUpperCase()})`);
    });

    // --- PERBAIKAN LOGIKA NAMA FILE SESUAI PERMINTAAN ---
    const namaProyek = filteredData[0].proyek.toUpperCase();
    const tglFile = now.toLocaleDateString('id-ID', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric'
    }).replace(/\//g, '-');

    const fileName = `REKAP_HSE_${range.toUpperCase()}_${namaProyek}_${tglFile}.pdf`;
    doc.save(fileName);
}

// 6. DASHBOARD & CRUD LOGIC (Standar)
function renderReports(filterData = reports) {
    ...
    filterData.forEach((data, index) => {
        ...
        const imgHTML = data.fotoAdmin 
            ? `<div class="img-table-container" onclick="openPreview(${originalIndex})">
            ...
            <tr>
                ...
                <td>
                    <div class="cell-content-enhanced">
                        ${imgHTML}
                        <div class="cell-text"><p>${data.keterangan.substring(0, 40)}...</p></div>
                    </div>
                </td>
...
function openPreview(index) {
    ...
    pb.innerHTML = `<img src="${d.fotoAdmin}" style="width:100%">${d.fotoInsiden ? `<img src="${d.fotoInsiden}" style="width:100%; border:2px solid red;">`:''}`;
