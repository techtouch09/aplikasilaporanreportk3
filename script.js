...
function showForm(index = -1) {
    ...
    if (index > -1) {
        ...
        // PERUBAHAN DISINI: Kosongkan agar bisa diinput manual
        // Atau biarkan kosong agar user memasukkan jam sesuai kejadian lapangan
        document.getElementById('waktu').value = ""; 
        document.getElementById('waktu').placeholder = "Contoh: 08:30 WIB";
    }
...
// --- FUNGSI HELPER KOMPRESI (TAMBAHKAN INI DI LUAR EVENT LISTENER) ---
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                // Ukuran maksimal 600px agar file sangat ringan (sekitar 50-100KB)
                const MAX_WIDTH = 600; 
...
document.getElementById('hse-form').addEventListener('submit', async (e) => {
    ...
    // Tambahan Validasi Waktu Manual
    if (!waktuInput || waktuInput.trim() === "") {
        alert("⚠️ Waktu lapor harus diisi secara manual!");
        return;
    }
    ...
    try {
        const reportData = {
            ...
            waktu: waktuInput, // Mengambil input manual dari user
            ...
            fotoAdmin: fotoAdminInput.files[0] ? 
                await processImage(fotoAdminInput.files[0]) : 
                (editIndex > -1 ? reports[editIndex].fotoAdmin : null),
                
            fotoInsiden: fotoInsidenInput.files[0] ? 
                await processImage(fotoInsidenInput.files[0]) : 
                (editIndex > -1 ? reports[editIndex].fotoInsiden : null)
        };
        ...
        console.error(error);
        alert("⚠️ GAGAL: Memori penuh. Hapus laporan lama.");
    }
});
